...
function ensureDirs() {
  for (const dir of [UPLOADS_ROOT, AUDIO_DIR, PHOTO_DIR]) {
...
function createUploader({ kind }) {
  const allowed = kind === "audio" ? AUDIO_MIME : PHOTO_MIME;
  const maxBytes = kind === "audio" ? AUDIO_MAX_BYTES : PHOTO_MAX_BYTES;
...
function persistUpload({ kind, file }) {
  ensureDirs();
  const targetDir = kind === "audio" ? AUDIO_DIR : PHOTO_DIR;
  ...
  const url = `/uploads/${kind === "audio" ? "audio" : "photos"}/${filename}`;
  return {
    url,
    sizeBytes: file.size,
    mime: file.mimetype,
