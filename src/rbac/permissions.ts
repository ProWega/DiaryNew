...
function isActiveAssignment(assignment: Assignment): boolean {
  return assignment.status !== "disabled";
}

function getAssignments(user: CurrentUser): Assignment[] {
...
function hasAssignment(user: CurrentUser, role: Role | null, subject: Subject = {}): boolean {
...
function hasRole(user: CurrentUser, role: Role): boolean {
  return user.role === role || user.baseRole === role || hasAssignment(user, role);
}

function sameSession(user: CurrentUser, subject: Subject): boolean {
  if (!subject.sessionId) {
    return true;
  }

  return (
    String(user.sessionId || "") === String(subject.sessionId) || hasAssignment(user, null, subject)
  );
}

function sameGroup(user: CurrentUser, subject: Subject): boolean {
  if (!subject.groupId) {
    return true;
  }

  return (
    String(user.groupId || "") === String(subject.groupId) || hasAssignment(user, null, subject)
  );
}
  ...
  switch (permission) {
    ...
    case "group.analytics.read":
      ...
      return (
        ...
        hasAssignment(user, "curator", subject) &&
        sameGroup(user, subject)
      );

    case "group.notes.write":
    case "group.export":
      return hasRole(user, "curator") && sameSession(user, subject) && sameGroup(user, subject);
...
export function getDefaultRoute(user: CurrentUser | null | undefined): string {
  if (!user) {
    return "/";
  }

  const participantAssignment = getAssignments(user).find((a) => a.role === "participant");
  const curatorAssignment = getAssignments(user).find((a) => a.role === "curator");
  const organizerAssignment = getAssignments(user).find((a) => a.role === "organizer");
...
export function getNavigationItems(user: CurrentUser | null | undefined): NavigationItem[] {
  if (!user) {
    return [];
  }
...
export function getScopeBadges(user: CurrentUser | null | undefined): string[] {
  if (!user) {
    return [];
  }
